...
function parsePrefilledLink(
  link: string,
): { actionUrl: string; emailFieldId: string } | null {
  if (!link.trim()) return null;
  try {
    const url = new URL(link.trim());
    const field = [...url.searchParams.keys()].find((key) =>
...
export const signup = {
  ...
  errorMalformed:
    "That doesn't look like a complete email address. Mind checking it?",
  errorNetwork:
    "Something went wrong on our end. Mind trying again in a moment?",

  privacyNote:
    "One email when we launch. No spam, no sharing your address, unsubscribe in a click.",
};
